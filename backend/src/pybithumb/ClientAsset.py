...
class ClientAsset(Process):
    ...
    def __init__(self, name):
        """
        save client's asset information in ticker dictionary
        ticker dict : ticker_dict['ticker'] = [total coin,in use coin,total KRW, in use KRM]
        """
        super().__init__()
        self.name = name
        ...
        self._tickers = ['ETH'] # for RUN CHECKING
        for ticker in tqdm(pybithumb.get_tickers()):
            ...
            if balance[0] > 0.0:
                ...
                self._tickers.append(ticker)# for RUN CHECKING
